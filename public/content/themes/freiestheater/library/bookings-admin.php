...
add_action( 'admin_menu', 'ft_bookings_menu' );

add_action('admin_enqueue_scripts', 'ft_admin_load_scripts');

function ft_admin_load_scripts($hook){
		global $ft_bookings_menu_page;

		wp_enqueue_script('angular','http://ajax.googleapis.com/ajax/libs/angularjs/1.2.15/angular.min.js', array(), null, false);
		wp_enqueue_script('angular-resource','http://ajax.googleapis.com/ajax/libs/angularjs/1.2.15/angular-resource.min.js', array(), null, false);
		wp_enqueue_script('ft-app',get_template_directory_uri() . '/angular/ft_admin.js', null, true);
		wp_localize_script('ft-app', 'ft_vars', array('ft_nonce'=> wp_create_nonce('ft_nonce')));
...
//
function ft_bookings_menu() {
//		add_menu_page( $page_title, $menu_title, $capability, $menu_slug, $function = '', $icon_url = '', $position = null )
		global $ft_bookings_menu_page;
		//create custom post type menu
		$ft_bookings_menu_page = add_menu_page( 'Reservierungen', 'Reservierungen', 'manage_options', 'reservierungen', 'ft_list_events' );
...
function ft_list_events() {


		echo '<pre>';
		var_dump(get_template_directory_uri(__FILE__));
		echo '</pre>';

		if (isset($_GET['date'])){
				// CREATE REQUESTED DATE
				$date = strtotime('01-'.$_GET['date']);
		}else{
				// DEFAULT DATE
				$date = strtotime('now');
		}
				...
				create_date_menu($date);
				$events= get_monthly_event_bookings($month, $year);

				$events = json_encode($events);

		if (isset($_GET['action'])){

				// admin.php?page=Reservierungen&date=02-2014&event_id=12512&booking_id=3&action=delete
				if ($_GET['action']==='delete'){
						// delete booking booking_id
						if ($_GET['booking_id']!==''){
							if(ft_delete_booking($_GET['booking_id'])){
								// REDIRECT
								// admin.php?page=Reservierungen&date=02-2014&event_id=12512&booking_id=3&action=delete
		...
		echo '<pre>';
		var_dump(admin_url( 'admin.php?page=Reservierungen&date=01-2014' ));
		echo '</pre>';
		echo '<pre>';
		var_dump($_REQUEST);
		echo '</pre>';
		echo '<hr>';
		// test update freie plaetze
		//$test = update_free_seats('9',143,'events_1_datum');
		...
		?>
		<div class="wrap" ng-app="ft_admin">


				<div ng-controller="EventCtrl">
				<h2>Reservierungen - {{bookings.data}}</h2>
				<hr>
				<div class="">

						<h2>11.05.2014 - Antigone </h2>';
						<table>
								<tr>
										<th>Tickets</th>
										<th>email</th>
...
function cpt_delete_post_type12() {
		global $CPT_URL;
...
function ft_check_return( $return ) {
